<template>
  <va-accordion class="sidebar-accordion va-sidebar__menu__inner" v-model="accordionValue" multiply>
    <va-collapse v-for="(route, idx) in items" :key="idx">
      <template #header>
        <va-sidebar-item :id="route.name + '-sidenav'" :active="isRouteActive(route)" :to="route.children ? undefined : { name: route.name }">
...
export default {
  ...
  methods: {
    ...
    isItemExpanded(item) {
      ...
      const isChildActive = !!item.children.find((child) => (child.children ? this.isItemExpanded(child) : this.isRouteActive(child)));
